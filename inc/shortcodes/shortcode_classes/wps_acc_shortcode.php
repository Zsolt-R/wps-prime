...
class WPS_Accordion_Shortcodes {
	...
	public function accordion_item_shortcode($atts, $content = null) {
		...
		$i_type = $options['ico_type'] ?$options['ico_type'] : 'fontawesome'; // Set as default iconfont

		$i_class = wps_getExtraClass($options['icon_class']);
		$i_class .= wps_getExtraClass($options["icon_{$i_type}"]);		

		wps_icon_element_fonts_enqueue( $i_type );
		...
		$accordion_title = sprintf('<%1$s id="%3$s" class="accordion-title%5$s" role="tab" aria-controls="%4$s" aria-selected="false" aria-expanded="false">%6$s%2$s</%1$s>',
			...
			$icon = $i_class ? "<i class=\"$i_class\"></i> " : ''
